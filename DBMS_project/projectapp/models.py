...
class Employee(models.Model):
    ...
    national_id_no = models.BigIntegerField(blank=False, unique=True)
...
class AccountHolder(models.Model):
    ...
    contact = models.BigIntegerField(blank=False, unique=True)
    address = models.CharField(max_length=200, blank=False)
    account_no = models.BigIntegerField(primary_key=True, unique=True, default=generate_account_no)
...
class Loan(models.Model):
    ...
    def __str__(self):
        return f'Loan for Account No: {self.account_holder.account_no}, Amount: {self.amount}'
